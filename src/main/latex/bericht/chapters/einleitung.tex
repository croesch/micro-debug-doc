...
Der Debugger soll in der Lage sein zwei Binärdateien einzulesen: eine Datei, die den \ma und eine, die den \ac enthält. Der \ac soll grundsätzlich nur vom \mac abhängig und durch den Benutzer frei definierbar sein; als Referenz für diese Arbeit soll aber der \gls{ijvm}-Assembler dienen.

Der Benutzer soll Breakpoints (Haltepunkte) definieren können, die die Simulation des \mac unterbrechen. Der Debugger soll dem Benutzer ermöglichen, Breakpoints für bestimmte \mais aber auch für bestimmte \ais zu definieren.

Neben den Werten der Register sollen dem Benutzer auch der Inhalt des Stacks, die gesetzten Breakpoints, der \ma und der \ac angezeigt werden können. Dem Benutzer soll es möglich sein, lokale Variablen, Register, den ausgeführten \ma sowie \ac zu beobachten.

\subsection*{GUI für den Debugger}
Im zweiten Teil der Aufgabe soll für den Debugger eine \gls{gui} entwickelt werden. Die \gls{gui} soll sowohl den \ma als auch den \ac anzeigen; Auch die Register mit ihren Werten und der Stack sollen sichtbar sein.

Die \gls{gui} soll dem Benutzer ermöglichen, Breakpoints zu setzen und soll anzeigen, welche \ma und \ai gerade abgearbeitet wird.
